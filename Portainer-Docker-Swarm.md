...
Run the following on master node to check the swarm status:
```
...
Now deploy the stack file on the master node:
```
...
Check container status on master and worker respectively:
```
pi@pi4-1:~ $ docker ps
